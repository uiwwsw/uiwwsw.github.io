...
- Easter egg (implementation spoiler): there is **no pre-discovery card, reception meter, hint, hold button or shortcut**. Normal momentum is discarded at Earth. Continued manual input encounters increasing resistance and three outward recoils; stopping loses unfinished progress. Deterministic simulations require about 46 seconds of maximal sustained input, or 68 seconds of repeated 300 px downward strokes on an 844 px viewport. These are model timings, not measured device feel. Only the final close approach reveals the GitHub / Velog destinations; auto-cruise and direct slider navigation never reveal them. The slider's forward keys and global forward travel keys use the same resistance. Reverse travel, “다시 별들 사이로”, `Esc`, or `Home` let you leave. The reveal is non-modal, never auto-opens an external link, and remains available without WebGL. Reduced motion removes coasting, camera lean and involuntary recoil/recession; recoil distance instead requires additional manual effort, not an instant shortcut.
