...
- Once the opening world settles, **4 seconds without action** reveals a small **조작 안내** cue. Pointer/key/wheel activity dismisses it; **14 seconds of renewed inactivity** brings it back. The home cue demonstrates wheel/pinch travel; while exploring it invites reading an actual star/title instead of encouraging accidental secret approach. The help button opens it immediately. Reading, hidden tabs, cruise, loading/failures and any Earth-focus signal cancel its pending timer and suppress the cue. Pointer capability and actual touch/mouse input choose the glyph. Only three animation cycles run; pause/reduced motion stay static, and the cue never captures input or steals focus. See [discovery and idle QA](design/discovery-idle-qa.md).
...
- Actual article titles become available just past 12% of the ordinary route, rather than waiting until roughly 41% for topic-cloud labels to switch to individual writing. Nebulae remain decorative; every title now belongs to the real star at its connector dot. If several stars sit under one tap, choose from the nearby-story list. Subject filters and the complete archive remain available.
- “탐험 구역” represents spatial capacity, not a subject or year classification. New writing fills the Moon–Earth route up to 96 reserved star slots, then numbered “기록 성운” regions of the same capacity. All current 59 articles belong to the home route. Year/topic changes do not move a star; “연도·주제로 찾기” searches the full archive across every region, with 24 stories per page. See [sector allocation QA](design/sector-allocation-qa.md).
...
- The native bottom slider has a **52px-high input area and 44×44px draggable thumb**, with a quiet 22px visual halo, a filled track, keyboard focus outline and 0.1% steps. The visible rail and Moon/Earth endpoint markers share the native thumb-center travel interval (22px inset on each side), all inside one full-width input. The handle meets both endpoints exactly; tapping either endpoint reaches 0%/100%. Its larger target is isolated from the canvas gesture recognizer; normal pinch and keyboard discovery behavior are retained. See [slider endpoint QA](design/slider-endpoints-qa.md). Sound starts muted and is generated locally with Web Audio.
...
`src/utils/observatory.js` builds metadata from the checked-in Velog archive. Categories are inferred from titles and code content. `src/data/sky-registry.json` stores star coordinates, numeric link aliases, and sectors. V2 repairs the old initial-batch split once: the original 58 home stars remain unchanged and the stranded 59th article moves home, retaining its ID and numeric alias. Thereafter the registry is append-only: new, reordered, edited or removed posts do not change existing saved coordinates or reuse old links. Deleted posts retain reserved slots so their eventual return does not move other stars. Visit-specific offsets are applied only at runtime. New posts use the first region with capacity, regardless of year/topic, and create a new numbered region only when all reserved slots are full.

`UniverseScene.jsx` handles travel and the procedural background; `CelestialBodies.jsx` renders Earth's day/night/cloud/atmosphere shaders and the lunar terrain. `ArticleSky.jsx` draws the active sector in one `THREE.Points` batch, plus at most four decorative topic nebula sprites. Picking uses the current camera every frame and a capped candidate list (36 mobile / 64 desktop), with the same near/edge visibility rule as the shader and real line-of-sight Earth occlusion. Bodies update before the camera, which updates before labels/picking. Layout is checked five times a second against measured UI bounds; titles try either side and upper/lower corners of their fixed star anchor. At most 3 / 5 labels are shown, with stable priority and 0.45-second retirement. A separate delta-based UI clock cannot be stranded when Fiber resets its absolute clock after hiding/resuming the tab. Title budgets are not star deletion: visible points without titles remain selectable. Saved coordinates, visit seeds and canonical links do not change.
...
Runtime optimization keeps the same visual quality: star projection is skipped before exploration, while reading an article and during Earth-only focus. Interactive frames reuse private per-sector projection records and candidate arrays; React labels and click results receive independent snapshots. Label-only Earth bounds run at 5 Hz, while hit coordinates still follow every interactive frame. Memoized Earth, nebula, background-star and dust components skip unchanged-prop React work during travel; their existing `useFrame` animation continues. Textures, particle counts, draw batches, lazy loading and saved star positions are unchanged. See [runtime optimization QA](design/runtime-optimization-qa.md); run `node scripts/benchmark-star-projection.js` for the isolated CPU comparison.
