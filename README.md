...
- Opening: the world assembles around a **fixed camera** in a brisk 1.8-second sequence after 3D readiness. Earth enters from entirely beyond the right edge immediately; the lunar floor and rocks start rising from below after 0.18 seconds. Both decelerate into place, with a short 0.32-second sky crossfade and no camera descent/tilt/zoom or changing sunlight. Offscreen offsets adapt to the viewport, including the atmosphere and terrain bounds. Optional cloud/night textures wait until entry has settled. Manual controls remain live, reduced motion/reading skip, and returning home does not replay. No new bitmap or draw batch is added; mobile ground remains 13,545 vertices. See [world assembly QA](design/world-assembly-qa.md) for the beat timings.
...
- Easter egg (implementation spoiler): there is **no pre-discovery card, reception meter, hint, hold button or shortcut**. After ordinary travel reaches Earth, fresh manual input transitions into a Korea-centered Earth-only close-up: writing labels, decorative stars/dust and the exploration HUD fade away, and free look gives way to the geographic focus camera. Continued input advances against increasing resistance **without any forced bounce**. After a 0.7-second finger-reset grace, stopping pulls the camera back to the original Earth orbit/scale (about 2.5–3.2 seconds including grace in simulations). Resuming input interrupts the return. Discovery still takes about 44 seconds of maximal sustained effort, and repeated forward pinches can also reveal it; these are model checks, not physical-device measurements. Only the final close approach reveals the GitHub / Velog destinations, which stay open once discovered. Auto-cruise and direct slider navigation never reveal them. Keyboard forward uses the same effort budget. Reverse, “다시 별들 사이로”, `Esc`, or `Home` let you leave; the reveal is non-modal and works without WebGL. Reduced motion removes coasting/lean and performs the release reset without an animated zoom-out.
...
- Even at rest, the universe moves: procedural nebula flow, independently shimmering stars, two-depth drifting dust, moving cloud shadows and a faint polar atmospheric shimmer. A brief distant light trail passes about every 18 seconds, first appearing after 3.5 seconds; trails never stack. These are artistic atmosphere effects, not a physical simulation or additional article stars. The Earth-focus mode fades the decorative sky out to keep Korea unobstructed.
