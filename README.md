...
- Opening: a 7.2-second lunar descent turns from the upper-left sky toward Earth. The left ridge and then ground enter from below; a restrained warm sidelight settles to the usual cool lunar light. It starts only after the real 3D scene is ready. Manual gestures/keyboard take over immediately with a short offset blend; reading, reduced motion and non-home navigation skip it. Returning home does not replay it. No new bitmap, postprocessing pass or second Moon is used; mobile ground vertices are reduced from 40,001 to 13,545. See [landing QA](design/landing-qa.md).
