...
function FragmentLabel({
  ...
  useFrame((state, delta) => {
    ...
    labelRef.current.visible = textRef.current.fillOpacity > 0.015 || hovered || isSelectedArticle;
  });
...
function ArticleCluster({
  ...
  const labelVisible = focusedArticleId === article.articleId || selectedSentence?.articleId === article.articleId;
  ...
  useFrame((state, delta) => {
    ...
    const pulse = 1 + Math.sin(state.clock.elapsedTime * 1.2 + article.articleId) * 0.08;
    const beaconScale = isSelected ? 1.35 : labelVisible ? 1.18 : 1;

    if (beaconRef.current) {
      beaconRef.current.scale.setScalar(beaconScale * pulse);
    }

    if (haloRef.current) {
      haloRef.current.scale.setScalar((beaconScale + 0.9) * pulse);
    }
  });
  ...
  return (
    <group ref={clusterRef} position={article.center}>
      <mesh ref={haloRef}>
        ...
        <meshBasicMaterial color={article.glow} transparent opacity={labelVisible ? 0.13 : 0.05} />
      </mesh>
      <mesh ref={beaconRef}>
        <sphereGeometry args={[1.15, 16, 16]} />
        <meshBasicMaterial color={article.color} transparent opacity={labelVisible ? 0.95 : 0.42} />
      </mesh>
