...
export function stepFlightMotion(
  ...
  if (allowSignal && state.distance >= 1 && state.distance < SIGNAL_DISTANCE) {
    ...
    if (state.pressure > 0) {
      ...
      // Forward input only advances: no invisible checkpoints can knock the
      // observer back while they are still approaching.
      state.distance = advanceFlight(state.distance, effort);
      state.pressure -= effort;
    } else if (state.time - state.lastPush > SIGNAL_RELEASE_GRACE) {
      // Finger-reset gaps are allowed; a real stop returns to the entry orbit.
      // Reduced motion performs the same reset without a zoom-out animation.
      state.distance = reducedMotion ? 1 : releaseSignal(state.distance, dt);
    }
  }
